...
module.exports = {
    ...
    run: async (client, message) => {
        ...
        if (wallet < 15000) return message.channel.send(`You need at least **15000** coins in your wallet!`);

        const chance = Math.floor(Math.random() * 2500) + 1;
        const chance_find = Math.floor(Math.random() * 2500) + 1;

        await message.channel.send(`**Are you sure you want to test your luck? This will cost __15000__ coins!**\nType \`y\` or \`n\` within the next 30 seconds to proceed.`)

        const filter = m => m.author.id === message.author.id && m.content.toLowerCase().includes("y") || m.content.toLowerCase().includes("n");
        let collector;
        ...
        try {
            ...
            return message.channel.send(
                "You did not respond with `n` or `y`!"
            );
        }
        ...
        if (first.toLowerCase() === "y") {
            if (chance === chance_find) {
                ...
                return message.channel.send("Congratulations! You won the lottery! **500000** coins have been added to your wallet.")
            } else {
                await UserEconomySchema.update(
                    { User: message.author.id },
                    {
